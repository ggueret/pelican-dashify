...
from .exceptions import (
    VideoProbeError, VideoStreamError, PackingError,
    AudioTranscodeError, VideoTranscodeError
)
...
def generate_video_representations(input_path, stream_info, settings):
    ...
    for i in range(settings["DASHIFY_VIDEO_REPRESENTATIONS"]):
        ...
        if output_width > settings["DASHIFY_VIDEO_MAX_WIDTH"] or \
           output_height > settings["DASHIFY_VIDEO_MAX_HEIGHT"]:
            logging.warning("Skip %dx%d representation of %s, resolution "
                            "greater then the max.",
                            output_width, output_height, input_path)
            continue

        if output_width < settings["DASHIFY_VIDEO_MIN_WIDTH"] or \
           output_height < settings["DASHIFY_VIDEO_MIN_HEIGHT"]:
            logging.warning("Skip %dx%d representation of %s, resolution "
                            "lower than the min.",
                            output_width, output_height, input_path)
            continue
        ...
        if output_bitrate > settings["DASHIFY_VIDEO_MAX_BITRATE"] or \
           output_bitrate < settings["DASHIFY_VIDEO_MIN_BITRATE"]:
            logging.warning("Skip %dx%d@%dk representation of %s, bitrate "
                            "limit reached.", output_width, output_height,
                            output_bitrate, input_path)
            continue
...
def load_input_info(path, settings):
    ...
    retcode, stdout, stderr = run_command(
        settings["DASHIFY_INFO_COMMAND"].format(
...
def transcode_video_stream(input_path, output_path, stream, width, height,
                           bitrate, settings):
    ...
    command = settings["DASHIFY_VIDEO_TRANSCODE_COMMAND"].format(
        ...
        maxrate=(bitrate * 2),
        bufsize=(bitrate * 4),
        keyint=(settings["DASHIFY_FRAMERATE"] *
                settings["DASHIFY_SEGMENT_DURATION"]),
...
def dashify_video(generator, content, input_relpath, keyword):
    ...
    if not os.path.isfile(input_path):
        raise VideoProbeError(
            "The file '{}' does not exist.".format(input_path))
    ...
    video_index = select_stream_by_typed_index(
        input_info["streams"],
        settings["DASHIFY_VIDEO_STREAM_INDEX"], codec_type="video")

    if video_index is None:
        raise VideoStreamError(
            "cannot find a valid video stream with index %{}".format(
                settings["DASHIFY_VIDEO_STREAM_INDEX"]))

    sexagesimal_parsed = re.match(
        settings["DASHIFY_SEXAGESIMAL_REGEX"],
        input_info["format"]["duration"]).groupdict()
    # fixme
    output_info["duration"] = datetime.timedelta(**{k: int(v) for k, v in sexagesimal_parsed.items()})  # noqa: E501
    ...
    if settings["DASHIFY_CACHE_PATH"]:
        cache_dir = os.path.abspath(
            os.path.join(settings["DASHIFY_CACHE_PATH"], input_relpath))
    else:
    ...
    output_presets = list(generate_video_representations(
    ...
    if not settings["DASHIFY_AUDIO_DISABLE"]:
        audio_index = select_stream_by_typed_index(
            input_info["streams"],
            settings["DASHIFY_AUDIO_STREAM_INDEX"], codec_type="audio")
        output_filename = "audio_{}k.mp4".format(
            settings["DASHIFY_AUDIO_BITRATE"])
        output_path = os.path.join(cache_dir, output_filename)
        output_paths.append(output_path + "#audio")

        if not os.path.exists(output_path):
            logging.info("Transcoding audio representation %s from %s",
                         output_filename, input_relpath)
            transcode_audio_stream(input_path, output_path, audio_index,
                                   settings["DASHIFY_AUDIO_BITRATE"], settings)
    ...
    for width, height, bitrate in output_presets:
        ...
        logging.info("Transcoding video representation %s from %s",
                     output_filename, input_relpath)
        transcode_video_stream(input_path, output_path, video_index, width,
                               height, bitrate, settings)
...
def discover_dashify(generator, content):
    ...
    for k, v in content.metadata.items():
        ...
        if isinstance(v, six.text_type) and \
           v.startswith(content.settings["DASHIFY_METATAG"]):
