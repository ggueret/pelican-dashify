...
def generate_video_representations(input_path, stream_info, settings):
    ...
    for i in range(settings["DASHIFY_VIDEO_REPRESENTATIONS"]):
        ...
        if (
            output_width > settings["DASHIFY_VIDEO_MAX_WIDTH"]
            or output_height > settings["DASHIFY_VIDEO_MAX_HEIGHT"]
        ):
            logging.warning(
                "Skip %dx%d representation of %s, resolution " "greater then the max.",
                output_width,
                output_height,
                input_path,
            )
        ...
        if (
            output_width < settings["DASHIFY_VIDEO_MIN_WIDTH"]
            or output_height < settings["DASHIFY_VIDEO_MIN_HEIGHT"]
        ):
            logging.warning(
                "Skip %dx%d representation of %s, resolution " "lower than the min.",
                output_width,
                output_height,
                input_path,
            )
        ...
        if (
            output_bitrate > settings["DASHIFY_VIDEO_MAX_BITRATE"]
            or output_bitrate < settings["DASHIFY_VIDEO_MIN_BITRATE"]
        ):
            logging.warning(
...
def load_input_info(path, settings):
    ...
    retcode, stdout, stderr = run_command(
        settings["DASHIFY_INFO_COMMAND"].format(
            ffprobe=settings["DASHIFY_FFPROBE_BIN"], input=path
        )
    )
...
def transcode_video_stream(
    input_path, output_path, stream, width, height, bitrate, settings
):
...
def dashify_video(generator, content, input_relpath, keyword):
    ...
    sexagesimal_parsed = re.match(
        settings["DASHIFY_SEXAGESIMAL_REGEX"], input_info["format"]["duration"]
    ).groupdict()
    # fixme
    ...
    if not settings["DASHIFY_AUDIO_DISABLE"]:
        audio_index = select_stream_by_typed_index(
            input_info["streams"],
            settings["DASHIFY_AUDIO_STREAM_INDEX"],
            codec_type="audio",
        )
    ...
    for width, height, bitrate in output_presets:
        ...
        logging.info(
            "Transcoding video representation %s from %s",
            output_filename,
            input_relpath,
        )
        transcode_video_stream(
            input_path, output_path, video_index, width, height, bitrate, settings
        )
